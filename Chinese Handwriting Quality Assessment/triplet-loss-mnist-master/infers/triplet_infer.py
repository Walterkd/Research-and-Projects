...
class TripletInfer(InferBase):
    def __init__(self, name=None, data=None, config=None):
        super(TripletInfer, self).__init__(data, config)
        if name:
    ...
    def test_dist(self):
        ...
        # (_, _), (X_test, y_test) = mnist.load_data_local()
        # since only load test data, no train data, array index of test data
        # change to start from 0, [0][0]: images, [0][1]: labels
        X_test = self.data[0][0]
        y_test = np.argmax(self.data[0][1], axis=1)
        # anchor = np.reshape(X_test, (-1, 64, 64, 1))
        anchor = np.reshape(X_test, (-1, 64, 64, 3))
        X = {
            'anc_input': anchor,
    ...
    def default_dist(self):
        ...
        X_test = self.data[0][0]
        y_test = np.argmax(self.data[0][1], axis=1)
        # y_test = self.data[0][1]  # gaidong: this is in format number? nope.
    ...
    def tb_projector(X_test, y_test, log_dir):
        ...
        with open(metadata, 'w') as metadata_file:
            for row in y_test:
                metadata_file.write('%d\n' % row[0])  # gaidong: only output a column, solve the problem.
