...
# from .py_file import some_class
...
class TripletModel(ModelBase):
    ...
    def triplet_loss_model(self):
        anc_input = Input(shape=(64, 64, 3), name='anc_input')  # anchor(gaidong: 1->3)
        pos_input = Input(shape=(64, 64, 3), name='pos_input')  # positive(gaidong: 1->3)
        neg_input = Input(shape=(64, 64, 3), name='neg_input')  # negative(gaidong: 1->3)
    ...
    def base_model(self):
        ...
        """
        ins_input = Input(shape=(64, 64, 3))  # 修改了原网络结构：(28,28,1)->(28,28,3)，为了和输入数据保持一致
