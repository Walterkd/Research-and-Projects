...
class TripletTrainer(TrainerBase):
    ...
    def show_acc_facets(y_pred, n, clz_size):
        ...
        :param n: 数据长度
        :param clz_size: 类别数量（此次实验中为2）
        :return: 打印数据
        """
        print("[INFO] trainer - n_clz: %s" % n)
        for i in range(clz_size):
            n = int(n)  # gaidong: float n -> int n, in case of typeError
            ...
            # 正样本和锚样本距离的平方和 - 负样本和锚样本距离平方和
            # 越小说明正样本离锚样本越近，训练效果越好
            r_count = basic_loss[np.where(basic_loss < 0)].shape[0]
            # 正样本更近的情况(basic_loss结果小于零）提取出来
